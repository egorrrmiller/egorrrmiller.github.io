(function () {
    ...
    const APPLETV_VERSION = '1.1.1';
    
    console.log('Apple TV UI: Script execution started');
    ...
    function injectBanner() {
        ...
        console.log('Apple TV UI: Attempting to inject banner into', active ? active.component : 'none');
        
        if (active && active.component === 'main') {
            const container = active.activity.render().find('.scroll__body').first();
            console.log('Apple TV UI: Scroll container status', container.length > 0 ? 'found' : 'not found');
            ...
            Lampa.TMDB.api('trending/all/day', (data) => {
                if (data && data.results && data.results.length > 0) {
                    ...
                    console.log('Apple TV UI: Rendering banner for', movie.title || movie.name);
                    renderHeroBanner(container, movie);
                }
            }, (error) => {
                console.log('Apple TV UI: TMDB API error', error);
            });
        }
    }
    ...
    function init() {
        ...
        Lampa.Listener.follow('activity', function (e) {
            ...
            if ((e.type === 'activity' || e.type === 'create') && e.component === 'main') {
        ...
        if (active && active.component === 'main') {
            console.log('Apple TV UI: Already on main at init, injecting banner');
